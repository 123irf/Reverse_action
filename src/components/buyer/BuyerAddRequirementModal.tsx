...
const BUYER_FORM_STORAGE_KEY = 'buyerAddRequirementFormData';
...
const AddRequirementModal: React.FC<AddRequirementModalProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    const loadSavedData = () => {
      try {
        const savedData = localStorage.getItem(BUYER_FORM_STORAGE_KEY);
        if (savedData) {
          ...
          if (isDataFresh && parsed.formData) {
            ...
            console.log('Loaded saved buyer form data from localStorage');
          } else {
            // Clear old data
            localStorage.removeItem(BUYER_FORM_STORAGE_KEY);
        ...
        console.error('Error loading saved buyer form data:', error);
        localStorage.removeItem(BUYER_FORM_STORAGE_KEY);
        const defaultTimes = getDefaultTimes();
        setFormData(prev => ({
          ...prev,
  ...
  useEffect(() => {
    ...
    const saveData = () => {
      try {
        ...
        localStorage.setItem(BUYER_FORM_STORAGE_KEY, JSON.stringify(dataToSave));
        setLastSaved(new Date());
        console.log('Buyer form data auto-saved to localStorage');
      } catch (error) {
        console.error('Error saving buyer form data:', error);
      }
    };
    ...
    const handleVisibilityChange = () => {
      if (document.hidden && isDataLoaded) {
        try {
          ...
          localStorage.setItem(BUYER_FORM_STORAGE_KEY, JSON.stringify(dataToSave));
          setLastSaved(new Date());
          console.log('Buyer form data saved due to tab visibility change');
        } catch (error) {
          console.error('Error saving buyer form data on visibility change:', error);
        }
      }
    };
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (isDataLoaded && hasFormData()) {
        try {
          ...
          localStorage.setItem(BUYER_FORM_STORAGE_KEY, JSON.stringify(dataToSave));
          console.log('Buyer form data saved due to page unload');
          ...
          console.error('Error saving buyer form data on page unload:', error);
        }
      }
    };
  ...
  const clearSavedData = () => {
    try {
      localStorage.removeItem(BUYER_FORM_STORAGE_KEY);
      console.log('Saved buyer form data cleared');
    } catch (error) {
      console.error('Error clearing saved buyer data:', error);
    }
  };
  ...
  const manualSave = () => {
    try {
      ...
      localStorage.setItem(BUYER_FORM_STORAGE_KEY, JSON.stringify(dataToSave));
      setLastSaved(new Date());
      console.log('Buyer manual save completed');
    } catch (error) {
      console.error('Error during buyer manual save:', error);
    }
  };
